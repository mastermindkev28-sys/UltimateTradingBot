"""
main.py — Ultra-Conservative Gold Futures Trading Bot
======================================================
Main orchestrator: initialises all components, receives TradingView webhook
signals, validates them, sizes positions, places bracket orders, and monitors
risk in real time.
...
Emergency stop:
    Send SIGINT (Ctrl+C) or POST /flatten with the webhook secret.
"""
...
class TradingBot:
    ...
    def __init__(self) -> None:
        ...
        self._last_daily_reset:  str      = ""    # date string "YYYY-MM-DD"

        mode = "🔴 LIVE" if not config.DEMO_MODE else "🟡 DEMO/PAPER"
        logger.info(
            "=" * 60
            + f"\n  UltimateTradingBot starting …"
            + f"\n  Mode: {mode}"
    ...
    async def initialize(self) -> None:
        ...
        # 3. Try to connect trading WebSocket (non-fatal if unavailable)
        try:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SIGNAL HANDLER  (called by WebhookServer for each incoming payload)
    # ══════════════════════════════════════════════════════════════════════════
    async def _on_signal_received(self, payload: dict) -> None:
        """Entry point for every incoming TradingView webhook payload."""
        ...
        if command == "pause":
            ...
            await self.telegram.send_pause_alert("Manual pause via webhook", self.risk.get_daily_stats())
            return
        if command == "resume":
    ...
    async def _process_trading_signal(self, payload: dict) -> None:
        """
        Full validation → sizing → order placement pipeline.
        Every gate is logged; a rejected gate simply returns early.
        """
        ...
        if not self.strategy.is_entry_allowed(now):
            logger.info("Gate 2 FAIL: entry not yet allowed (OR window)")
            return
        ...
        # ── Gate 5: Daily profit target (stop early when ahead) ───────────
        if self.risk.is_daily_profit_target_hit():
            logger.info(
                "Gate 5: Daily profit target reached (%.2f%%). "
                "No new trades today.",
        ...
        if self.risk.daily_trades >= config.MAX_TRADES_PER_DAY:
            logger.info(
                "Gate 6 FAIL: max trades/day (%d) reached",
        ...
        # ── Gate 9: No existing position in this instrument ───────────────
        positions = await self.tradovate.get_positions(self.account_id)
        instrument = payload.get("instrument", config.DEFAULT_INSTRUMENT).upper()
        if positions:
            logger.info("Gate 9 FAIL: position already open — %s", positions)
            return

        # ── Gate 10: Strategy validation ──────────────────────────────────
        signal = self.strategy.parse_and_validate_signal(payload)
        if signal is None:
            logger.info("Gate 10 FAIL: strategy validation failed")
            return
        ...
        potential_gain = sizing["risk_amount"] * config.TP1_R   # conservative estimate
        ...
        # Split into 2 legs for scale-out
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # POSITION MONITOR  (runs every POSITION_POLL_INTERVAL seconds)
    # ══════════════════════════════════════════════════════════════════════════
    async def _monitor_loop(self) -> None:
        """Continuous monitoring: time stops, equity refresh, drawdown checks."""
        while self.running:
            try:
                now    = datetime.now(self.tz)
                equity = await self.tradovate.get_account_equity(self.account_id)
                ...
                if self.strategy.is_force_close_time(now):
                    ...
                    if positions:
                        ...
                        # Record time-stopped trade
                        self.risk.record_trade_exit(
                            exit_price=0,  # actual P&L computed by Tradovate
                            pnl=0,
                            status="time_stop",
                        )
                ...
                if now.hour == 15 and now.minute == 5:
                    await self.telegram.send_daily_summary(self.risk.get_daily_stats())
    ...
    async def _flatten_all(self, reason: str = "Emergency") -> None:
        """Market-close all open positions immediately."""
